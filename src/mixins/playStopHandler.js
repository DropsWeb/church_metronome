...
const metronomeHandler = {
    ...
    methods: {
        createLoop() {
            ...
            this.loop = new Tone.Loop(time => {
                if (this.beatCounter == this.curVals.beats) this.beatCounter = 0;
                if (this.beatCounter == 0 && this.curVals.sFirstBeat) {
                    this.sndHigh.start(time)
                } else {
                    this.sndLow.start(time)
                }
                let visible;
                (document.visibilityState === 'hidden') ? visible = false: visible = true;
        ...
        // Визуальное отображение бита метронома
        visualStart(clsName) {
            ...
            if (beat && beat.classList.contains('sFbeat')) {
                beat.classList.add('sfbGlow')
            } else if (beat) {
                beat.classList.add('beatGlow')
            }
        },
        visualStop(clsName) {
            console.log(5);
